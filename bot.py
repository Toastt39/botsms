...
class TopupModal(nextcord.ui.Modal):
    def __init__(self):
        super().__init__('เติมเงินเข้าสู่ระบบ')
        self.a = nextcord.ui.TextInput(
            label='ลิ้งค์ซองอังเปา',
            placeholder='กรอกลิ้งซองอังเปา💸💰︱URL',
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if re.match(r'https:\/\/gift\.truemoney\.com\/campaign\/\?v=+[a-zA-Z0-9]{18}', link):
            ...
            try:
                ...
                embed = nextcord.Embed(description='❌  ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้', color=nextcord.Color.red())
                ...
                for roleData in config.roleSettings:
                    if (amount == roleData['price']):
                        ...
                        if (role in interaction.user.roles):
                            ...
                            embed = nextcord.Embed(
                                description=f'✅  ซื้อบทบาทนี้สำเร็จ',
                                color=nextcord.Color.from_rgb(0, 255, 0)
                            )
                        await bot.get_channel(int(config.LogTopup)).send(embed=nextcord.Embed(
                            description=f'''
`💸` **`ซื้อยศสำเร็จ`**
> `ราคายศ` : `{amount}` บาท
...
''',
                        ...
                        break
                if (embed is None):
                    embed = nextcord.Embed(description='❌ จำนวนเงินในซองผิดพลาด', color=nextcord.Color.from_rgb(255, 0, 0))
            else:
                print(response.text)
                embed = nextcord.Embed(description='❌ มีบางอย่างผิดพลาดขณะนี้', color=nextcord.Color.from_rgb(255, 0, 0))
        else:
            embed = nextcord.Embed(description='❌ ลิ้งซองอังเปาของท่านไม่ถูกต้อง', color=nextcord.Color.from_rgb(255, 0, 0))
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
@bot.slash_command(
    name='setup-premium',
    description='📍︱ตั้งระบบยิงเบอร์︱คำสั่งนี้สำหรับผู้ดูแลระบบเท่านั้น',
    guild_ids=config.server
)
async def setuppremium(interaction: nextcord.Interaction):
    ...
    embedsell = nextcord.Embed()
    embedsell.title = f'**SMS VIP**'
    embedsell.description = f'''
**วิธีใช้งานคำสั่ง**
```fix
/sms phone|เบอร์ amount|จำนวน```
**ตัวอย่าง วิธีการใช้งาน **
```fix
/sms 0923153995 10```
'''
    embedsell.set_image(url='https://cdn.discordapp.com/attachments/1149949359824318545/1290523240049147904/standard.gif?ex=66fcc4ed&is=66fb736d&hm=d48cfe93a41d204ec6257a89a1afb225c46c8783b10bd657feb7691f2be4f525&')
    embedsell.color = nextcord.Color.from_rgb(100, 220, 255)
    await interaction.channel.send(embed=embedsell, view=BuyView())
    await interaction.response.send_message(content='[SUCCESS] ตั้งค่าระบบสำเร็จเรียบร้อย', ephemeral=True)

@bot.slash_command(
    name='sms',
    description='🎲︱ยิงเบอร์︱คำสั่งนี้สำหรับสมาชิกวีไอพี',
    guild_ids=config.server
)
async def sms(interaction: nextcord.Interaction, phone: str, amount: int):
    if interaction.channel.id == config.channelid:
        if amount <= config.limit:
            await bot.get_channel(config.channelLog).send(embed=nextcord.Embed(
                title="LOG SMS",
                description=f'''
...
Number : {phone}
Amount : {amount}
Times : {datetime.now(bangkok_tz).strftime('%d/%m/%Y %H:%M:%S')}
```
''',
                color=nextcord.Color.green()
            ).set_footer(text="© MADE BY  :  ._toastt"))

            embedsms = nextcord.Embed()
            embedsms.title = f'`🎄` `SERVER SMS`'
            embedsms.description = f'''
> `📍` `ผู้ใช้` <@{interaction.user.id}>
> `🎲` `เบอร์เป้าหมาย` {phone[:4]}XXXX
> `🎲` `จำนวน` {amount} `วินาที`
> `⏳` `สถานะ` `กำลังดำเนินการ...`
'''
            embedsms.color = nextcord.Color.green()
            embedsms.set_image(url='https://cdn.discordapp.com/attachments/1149949359824318545/1290523240049147904/standard.gif?ex=66fcc4ed&is=66fb736d&hm=d48cfe93a41d204ec6257a89a1afb225c46c8783b10bd657feb7691f2be4f525&')
            embedsms.set_thumbnail(url='https://cdn.discordapp.com/attachments/970624157966540851/1051400565432782878/70a552e8e955049c8587b2d7606cd6a6.gif?ex=66fc4dd4&is=66fafc54&hm=7055a2298fa04a01aab609a6a18f115a166c6d94b4db71e693535a16cb9bbc36&')


            await interaction.response.send_message(embed=embedsms, ephemeral=True)
            ...
            for i in range(amount):
                ...
                threading.Thread(target=send_sms, args=(phone,)).start()
...
'''
            await interaction.edit_original_message(embed=embedsms)

        else:
            embed_limit = nextcord.Embed(
                title="`❌` MAX LIMITS",
                description=f"> ```จำนวน {amount} เกินกว่าที่กำหนด```",
                color=nextcord.Color.red()
            )
            await interaction.response.send_message(embed=embed_limit, ephemeral=True)
    else:
        embed_channel = nextcord.Embed(
            title="`❌` WRONG ROOM",
            description="```คำสั่งนี้ไม่สามารถใช้ได้ในช่องนี้```",
            color=nextcord.Color.red()
        )
        await interaction.response.send_message(embed=embed_channel, ephemeral=True)

@bot.event
async def on_slash_command_error(interaction: nextcord.Interaction, error):
    if isinstance(error, commands.CommandNotFound):
        await interaction.response.send_message('❌ This commands is Invild!', ephemeral=True)
